...
- 失败统一 detail='鉴权失败'，不泄露具体 reason（jti_replay / cross_flow / role_mismatch / sub_mismatch / token_expired）
- 集成测试用真 Redis + 真 PG 不 mock（CLAUDE.md §2.3 + 用户 memory feedback）

#### REQ Status
- AUTH-01 完成（JWT 签发 + payload schema）
- AUTH-02 完成（/api/auth/exchange + HttpOnly cookie + role 路由）
- AUTH-03 完成（jti SET NX EX + 节点失效 hook）
- AUTH-04 完成（(flow_id, node_id, sub) 三元组 + role 校验 + cross_flow 拒绝）
