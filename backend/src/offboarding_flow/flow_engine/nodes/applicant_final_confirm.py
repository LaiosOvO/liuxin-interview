...
2. 仅 advance / return 两态决策，无 reject（PRD §4.5.2）
3. 默认 actor = state.employee_id（申请人本人）
4. 默认 result_text 视 action 而定（无异议已确认 / 有异议退回 HR 终审）
"""
...
async def applicant_final_confirm_node(state: OffboardingState) -> dict[str, Any]:
    """申请人最终确认节点 — 聚合 timeline 并 interrupt 等申请人决策。"""
